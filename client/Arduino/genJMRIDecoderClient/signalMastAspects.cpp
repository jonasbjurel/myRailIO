...
signalMastAspects::signalMastAspects(const lgLink* p_parentHandle) {
    ...
    LOG_INFO("signalMastAspects::signalMastAspects: Signal mast aspect object %d for lgLink: %d created" CR, this, lgLinkNo);
}
...
rc_t signalMastAspects::onConfig(const tinyxml2::XMLElement* p_smAspectsXmlElement) {
    LOG_INFO("signalMastAspects::onConfig: Configuring Mast aspects" CR);
    ...
    for (uint8_t i = 0; true; i++) {
        LOG_INFO("signalMastAspects::onConfig: Parsing Signal mast Aspect: %s" CR, smAspectsXmlElement->FirstChildElement("AspectName")->GetText());
        ...
        for (uint16_t j = 0; true; j++) {
            ...
            LOG_INFO("signalMastAspects::onConfig: Parsing Mast type %s for Aspect %s" CR, mastTypeAspectXmlElement->FirstChildElement("Type")->GetText(), smAspectsXmlElement->FirstChildElement("AspectName")->GetText());
            ...
            if (mastAlreadyExist) {
                LOG_WARN("signalMastAspects::onConfig: Parsing Mast type %s for Aspect %s already exists, skipping..." CR, mastTypeAspectXmlElement->FirstChildElement("Type")->GetText(), smAspectsXmlElement->FirstChildElement("AspectName")->GetText());
                break;
            }
            else { //Creating a new signal mast type'
                LOG_INFO("signalMastAspects::onConfig: Creating Mast type %s" CR, mastTypeAspectXmlElement->FirstChildElement("Type")->GetText());
            ...
            LOG_TERSE("signalMastAspects::onConfig: Adding Asspect %s to MastType %s" CR, smAspectsXmlElement->FirstChildElement("AspectName")->GetText(), mastTypeAspectXmlElement->FirstChildElement("Type")->GetText());
            for (uint8_t p = 0; p < SM_MAXHEADS; p++) {
                if (headXmlElement == NULL) {
                    LOG_INFO("signalMastAspects::onConfig: No more Head appearances, padding up with UNUSED_APPEARANCE from Head %d" CR, p);
                ...
                if (!strcmp(headXmlElement->GetText(), "LIT")) {
                    LOG_TERSE("signalMastAspects::onConfig: Adding LIT_APPEARANCE for head %d, MastType %s and Appearance %s" CR, p, mastTypeAspectXmlElement->FirstChildElement("Type")->GetText(), smAspectsXmlElement->FirstChildElement("AspectName")->GetText());
                    aspects.back()->mastTypes.back()->headAspects[p] = LIT_APPEARANCE;
                }
                if (!strcmp(headXmlElement->GetText(), "UNLIT")) {
                    LOG_TERSE("signalMastAspects::onConfig: Adding UNLIT_APPEARANCE for head %d, MastType %s and Appearance %s" CR, p, mastTypeAspectXmlElement->FirstChildElement("Type")->GetText(), smAspectsXmlElement->FirstChildElement("AspectName")->GetText());
                    aspects.back()->mastTypes.back()->headAspects[p] = UNLIT_APPEARANCE;
                }
                if (!strcmp(headXmlElement->GetText(), "FLASH")) {
                    LOG_TERSE("signalMastAspects::onConfig: Adding FLASH_APPEARANCE for head %d, MastType %s and Appearance %s" CR, p, mastTypeAspectXmlElement->FirstChildElement("Type")->GetText(), smAspectsXmlElement->FirstChildElement("AspectName")->GetText());
                    aspects.back()->mastTypes.back()->headAspects[p] = FLASH_APPEARANCE;
                }
                if (!strcmp(headXmlElement->GetText(), "UNUSED")) {
                    LOG_TERSE("signalMastAspects::onConfig: Adding UNUSED_APPEARANCE for head %d, MastType %s and Appearance %s" CR, p, mastTypeAspectXmlElement->FirstChildElement("Type")->GetText(), smAspectsXmlElement->FirstChildElement("AspectName")->GetText());
    ...
    //End outer loop
    LOG_INFO("signalMastAspects::onConfig: Signal mast aspects successfully configured" CR);
    return RC_OK;
}

void signalMastAspects::dumpConfig(void) {
    LOG_INFO("signalMastAspects::dumpConfig: <Aspect config dump Begin>" CR);
    for (uint8_t i = 0; i < aspects.size(); i++) {
        LOG_INFO("signalMastAspects::dumpConfig:     <Aspect: %s>" CR, aspects.at(i)->name);
        for (uint8_t j = 0; j < aspects.at(i)->mastTypes.size(); j++) {
            LOG_INFO("signalMastAspects::dumpConfig:        <Mast type: %s>" CR, aspects.at(i)->mastTypes.at(j)->name);
            for (uint8_t k = 0; k < SM_MAXHEADS; k++) {
                switch (aspects.at(i)->mastTypes.at(j)->headAspects[k]) {
                case LIT_APPEARANCE:
                    LOG_INFO("signalMastAspects::dumpConfig:            <Head %d: LIT>" CR, k);
                    break;
                case UNLIT_APPEARANCE:
                    LOG_INFO("signalMastAspects::dumpConfig:            <Head %d: UNLIT>" CR, k);
                    break;
                case FLASH_APPEARANCE:
                    LOG_INFO("signalMastAspects::dumpConfig:            <Head %d: FLASHING>" CR, k);
                    break;
                case UNUSED_APPEARANCE:
                    LOG_INFO("signalMastAspects::dumpConfig:            <Head %d: UNUSED>" CR, k);
                    break;
                default:
                    LOG_INFO("signalMastAspects::dumpConfig:            <Head %d: UNKNOWN>" CR, k);
                    break;
                }
            }
        }
...
rc_t signalMastAspects::getAppearance(char* p_smType, char* p_aspect, uint8_t** p_appearance) {
    if (parentHandle->systemState::getOpStateBitmap() & OP_UNCONFIGURED) {
        LOG_ERROR("signalMastAspects::getAppearance: Aspects not configured, doing nothing..." CR);
    ...
    for (uint8_t i = 0; true; i++) {
        if (i > aspects.size() - 1) {
            LOG_ERROR("signalMastAspects::getAppearance: Aspect doesnt exist, setting mast to failsafe appearance and continuing..." CR);
            *p_appearance = failsafeMastAppearance;
            return RC_GEN_ERR;
        }
        if (!strcmp(p_aspect, aspects.at(i)->name)) {
            for (uint8_t j = 0; true; j++) {
                if (j > aspects.at(i)->mastTypes.size() - 1) {
                    LOG_ERROR("signalMastAspects::getAppearance: Mast type doesnt exist, setting to failsafe appearance and continuing..." CR);
                    *p_appearance = failsafeMastAppearance;
                    return RC_GEN_ERR;
                }
...
rc_t signalMastAspects::getNoOfHeads(const char* p_smType, uint8_t* p_noOfHeads, bool p_force) {
    if ((parentHandle->systemState::getOpStateBitmap() != OP_WORKING) && !p_force) { //NEEDS ATTENTION
        LOG_ERROR("signalMastAspects::getNoOfHeads: OP_State is not OP_WORKING, doing nothing..." CR);
        *p_noOfHeads = 0;
        return RC_GEN_ERR;
    }
    for (uint8_t i = 0; i < aspects.size(); i++) {
    ...
    LOG_ERROR("signalMastAspects::getNoOfHeads: Mast type not found, doing nothing..." CR);
    *p_noOfHeads = 0;
    return RC_GEN_ERR;
}
