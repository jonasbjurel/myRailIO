...
/* Description                                                                                                                                  */
/*==============================================================================================================================================*/
// This is the hardware driver for the genJMRI satelite Hardware (in short satelite) which is a model railway sensor and actuator decoder.
// This project is part of a larger JMRI project called genJMRIdecoder project, which is found here XXXXXX.
// Several satelites (up to #MAX_NO_OF_SAT_PER_CH) can be daisy chained in a loop for each satelite link. 
// Each of the satelites can produce 4 actuator outputs, each of them individually configured through satelite link commands (E.g. PWM, Pulses,
// and static low/high satetes. They also collect up to 8 sensor inputs - transfered to the master through the satelite link.
// The satelite link commands transport to all satelites is protected by CRC checksums, separate checksums for each satelite. Whenever a
// satelite detects a checksum error, it will forward a feedback message to the master indicating that it detected a checksum error.
// Each satelite also produce a CRC checksum to protect the data it is returning to the link master.
// Principle:
//
//   +-------------+        A         +----------+       A+B      +----------+
//   !             !--------/-------->! Satelite !--------/-------! Satelite !--------//---------+
...
// L1 protocol:
//
// A: StartMARK XXXXXXX Satelite commands (cmd) and error supervision: Actuator mode(actMode), Actuator values(actVal), Invalidated Checksum generated from
//    the link master towards a satelite(invServerCrc - which will result in a remote CRC error generated by the particular satelite subjec
//    to the CRC-error), Order to invalidate outgoing Satelite CRC-checksum error towars the link master(invClientCrc, which will result in
//    Linkmaster receive CRC Error(XXXXXXXXXXXXXXXXXXXXXXX)
//
// The link L1 protocol mimimcs the WS28XX protocol:
//
// Link data structure:
...
#define WS28XX_T0H_CYC_MAX          35              //WS28XX_T0H_CYC * 1.1
...
typedef void (*satLinkStateCb_t)(satOpState_t);
typedef void (*satStateCb_t)(uint8_t LinkAddr_p, uint8_t SatAddr_p, satOpState_t satOpState);
typedef void (*satSenseCb_t)(uint8_t LinkAddr_p, uint8_t, uint8_t SatAddr_p, bool senseVal_p);
...
struct satInfo_t {
    ...
    sensor_t sensors[8];                // Sensor struct
...
//Error codes
#define SAT_OK                        0
#define SAT_ERR_SYMBOL_ERR            1
#define SAT_ERR_EXESSIVE_SATS_ERR     2
#define SAT_ERR_GEN_SATLINK_ERR       3
#define SAT_ERR_WRONG_STATE_ERR       4
#define SAT_ERR_SAT_NOTBLOCKED_ERR    5
#define SAT_ERR_PARAM_ERR             6
#define SAT_ERR_RMT_ERR               7
#define SAT_ERR_EXESSIVE_SATS         8
#define SAT_ERR_SCANTASK_ERR          9
...
public:
    ...
    void getSatLinkStats(satPerformanceCounters_t* satStats_p, bool resetStats);
...
public:
    ...
    satErr_t setSenseFilter(uint8_t senseFilter_p, uint8_t actIndex_p);
