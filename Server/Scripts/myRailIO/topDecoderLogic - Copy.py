...
class topDecoder(systemState, schema):
    def __init__(self, win, demo=False):
        ...
        self.commitAll()
        self.MQTTalarm = alarm(self)
        self.MQTTalarm.type = "CONNECTION STATUS"
        self.MQTTalarm.src = self.nameKey.value
        self.MQTTalarm.criticality = ALARM_CRITICALITY_A
        self.MQTTalarm.sloganDescription = "MQTT client disconnected"
        self.RPCalarm = alarm(self)
        self.RPCalarm.type = "CONNECTION STATUS"
        self.RPCalarm.src = self.nameKey.value
        self.RPCalarm.criticality = ALARM_CRITICALITY_A
        self.RPCalarm.sloganDescription = "RPC client disconnected"
        self.LOGLEVELalarm = alarm(self)
        self.LOGLEVELalarm.type = "PERFORMANCE WARNING"
        self.LOGLEVELalarm.src = self.nameKey.value
        self.LOGLEVELalarm.criticality = ALARM_CRITICALITY_B
        self.LOGLEVELalarm.sloganDescription = "High log-level enabled, this may lead to extensive system load and excessive performance degradation"
    ...
    def __sysStateAllListener(self, changedOpStateDetail, p_sysStateTransactionId = None):
        ...
        if opStateDetail & OP_DISABLED[STATE]:
            #self.MQTTalarm.ceaseAlarm("The object is manually disabled/adminstratively blocked - and therefore removed from the active alarm list")
            #self.RPCalarm.ceaseAlarm("The object is manually disabled/adminstratively blocked - and therefore removed from the active alarm list")
            ...
            pass
        if opStateDetail & OP_DISCONNECTED[STATE]:
