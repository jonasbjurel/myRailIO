...
class satelite(systemState, schema):
    ...
    def addChild(self, resourceType, name = None, config = True, configXml = None, demo = False):
        if resourceType == SENSOR:
            ...
            print(self.sensors.candidateValue)
            self.childs.value = self.sensors.candidateValue + self.actuators.candidateValue
            print("YYYYYYYYYYYYYYYYYY")
            print(self.sensors.candidateValue)
            ...
            if not config and configXml:
                ...
                self.reEvalOpState()
                if res != rc.OK:
                    trace.notify(DEBUG_ERROR, "Failed to configure Sensor: " + nameKey.candidateValue + " - return code: " + rc.getErrStr(res))
                    return res
                print("###########################")
                print(self.sensors.candidateValue)
                trace.notify(DEBUG_INFO, "Sensor: " + self.sensors.candidateValue[-1].nameKey.candidateValue + " successfully added to satelite link " + self.nameKey.candidateValue)
                return rc.OK
            if config:
    ...
    def __validateConfig(self):
        ...
        if res:
            trace.notify(DEBUG_ERROR, "Sensor failed address/port topology validation for port/address: " + str(self.satLinkAddr.candidateValue) + rc.getErrStr(res))
            return res
        return rc.OK
