...
class actuator(systemState, schema):
    def __init__(self, win, parentItem, rpcClient, mqttClient, name=None, demo=False):
        ...
        self.NOT_CONNECTEDalarm.sloganDescription = "Actuator reported disconnected"
        ...
        self.NOT_CONFIGUREDalarm.sloganDescription = "Actuator has not received a valid configuration"
        ...
        self.INT_FAILalarm.sloganDescription = "Actuator has experienced an internal error"
        self.CBLalarm = alarm()
    ...
    def __sysStateAllListener(self, changedOpStateDetail):
        ...
        if opStateDetail & OP_INIT[STATE]:
            self.NOT_CONNECTEDalarm.raiseAlarm("Actuator has not connected, it might be restarting-, but may have issues to connect to the WIFI, LAN or the MQTT-brooker")
        else:
            self.NOT_CONNECTEDalarm.ceaseAlarm("Actuator has now successfully connected")
        if (opStateDetail & OP_UNCONFIGURED[STATE]):
            self.NOT_CONFIGUREDalarm.raiseAlarm("Actuator has not been configured, it might be restarting-, but may have issues to connect to the WIFI, LAN or the MQTT-brooker, or the MAC address may not be correctly provisioned")
        else:
            self.NOT_CONFIGUREDalarm.ceaseAlarm("Actuator is now successfully configured")
        if (opStateDetail & OP_INTFAIL[STATE]):
            self.INT_FAILalarm.raiseAlarm("Actuator is experiencing an internal error")
        else:
